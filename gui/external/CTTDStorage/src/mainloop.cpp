...
void MainLoop::showMsgBox(QString msg, int dlgType)
{
...
void MainLoop::initAgentTestInfoModel()
{
    ...
    {
        QString agnetInfo = "{\"AgentList\":[{\"AgentIP\":\"118.89.165.39\",\"AgentAddr\":\"未知 \",\"PaymentMethod\":\"0\",\"Price\":\"1/1\"},{\"AgentIP\":\"212.129.144.237\",\"AgentAddr\":\"未知  \",\"PaymentMethod\":\"0\",\"Price\":\"1/1\"},{\"AgentIP\":\"127.0.0.1\",\"AgentAddr\":\"未知   \",\"PaymentMethod\":\"0\",\"Price\":\"1/1\"},{\"AgentIP\":\"111.229.236.194\",\"AgentAddr\":\"未知  \",\"PaymentMethod\":\"0\",\"Price\":\"1/1\"}]}";        AgentDetailsFunc::ParseAgentInfo(agnetInfo, m_AgentDetails, m_AgentTestInfo);
...
{
    return m_AgentDetails[m_AgentConnectedAccount].FlowAmount;
}

QString MainLoop::getCurrAgentFlowRemain()
{
    return m_AgentDetails[m_AgentConnectedAccount].FlowUsed;
}

void MainLoop::setServiceState(bool state)
{
    m_AtServiceFlag = state;
...
bool MainLoop::doWaitTXCompleteOp(QString TXtoMonitor, GUIString::RpcCmds cmdType)
{
    if (cmdType == GUIString::SignAgentCmd || cmdType == GUIString::CancelAgentCmd)
    {
                ...
                ret.compare(m_GuiString->sysMsgs(GUIString::OpTimedOutMsg).toStdString()) == 0)
        {
            ...
            setServiceState(cmdType == GUIString::SignAgentCmd);
        //m_BusyFlag = false;
...
//    {
//        accParam.clear();
//        cmd = GUIString::GetAccTBalanceCmd;
//        errHint = m_GuiString->uiHints(GUIString::GetAccTBalanceErrHint);
//    }
//    else
//    {
//        accParam.push_back(account);
...
{
    if (bal.compare(m_GuiString->sysMsgs(GUIString::OpTimedOutMsg)) == 0)
        return;
...
//        return;
    ...
    if (ret.compare(m_GuiString->sysMsgs(GUIString::ErrorMsg).toStdString()) == 0 ||
        ...
        return (m_GuiString->uiHints(GUIString::TxSuccessfullySentHint) + ", 交易号: " + QString::fromStdString(ret));
}

QString MainLoop::applyForAgentService(QString agentAccount)
{
    QString start, stop, flow;
    QString timePeriod, flowAmount, ret;
    flowAmount = "0"; //QString::number(flow.toLongLong() * GB_Bytes);
    timePeriod = "2020-01-01 00:00:00~2020-12-31 23:59:59"; //start + "~" + stop;
    ...
    {
        std::thread t(bind(&MainLoop::updateAgentDetails, this, agentAccount, ret, GUIString::SignAgentCmd, true));
        t.detach();
...
QString MainLoop::createSpace(QString spaceLabel, QString spaceSize)
{
    string ret = m_BackRPC->SendOperationCmd(GUIString::CreateObjCmd, vector<QString>{QString::number(m_ObjInfos.size()+1),
                QString::number(spaceSize.toLongLong() * MB_Bytes), m_AccountPwd});
    if (ret.compare(m_GuiString->sysMsgs(GUIString::ErrorMsg).toStdString()) == 0 ||
            ret.compare(m_GuiString->sysMsgs(GUIString::OpTimedOutMsg).toStdString()) == 0)
...
QString MainLoop::downloadFileByIndex(QString filePath, int spaceIndex, int fileIndex)
{
    ...
    {
        //emit ShowDownFileOpRetSign(m_GuiString->uiHints(GUIString::NeedMoreFlow));
                                     ...
                                     QString startTime, QString stopTime, QString price)
{
    ...
    QString sharingCode = generateSharingCode(objID, fName, offset, length, receiver,
                                              startTime, stopTime, price.toLongLong(), sharerAgentNodeID);
...
QString MainLoop::getSharingInfo(QString sharingCode)
{
    ...
    setSharingFileInfo(QString::fromStdString(sharer), fName, startTime, stopTime, QString::number(price));
...
void MainLoop::setSpaceInfo(qint64 totalSpace, qint64 remainSpace)
{
    ...
    {
         m_TotalSpace = m_GuiString->uiHints(GUIString::NotAvailableHint);
         m_UsedSpace = m_GuiString->uiHints(GUIString::NotAvailableHint);
    }
    emit totalSpaceChanged(totalSpace);
    emit usedSpaceChanged(totalSpace - remainSpace);
}

void MainLoop::waitingForNewSpaceCreating()
{
    ...
    if (timeoutCnt >= timeOutLimit)
        ...
        emit showMsgBoxSig(m_GuiString->uiHints(GUIString::CreateSpaceSuccessHint), GUIString::HintDlg);
}
...
bool MainLoop::readMetaDataByObjID(qint64 objID, qint64& spaceTotal, qint64& spaceRemain, QString& result)
{
    ...
    {
        result = m_GuiString->uiHints(GUIString::ParseMetaErrHint);
        return false;
    }
...
{
    QString data = label + CTTUi::spec + QString::number(size) + CTTUi::spec + QString::number(size - fHeadLength);
...
void MainLoop::doUploadFileOp(qint64 objID, qint64 offset, QString fileName, qint64 fileSize, QString filePath)
{
    ...
    if (!updateMetaDataOp(objID, fileName, fileSize, offset, updateMetaDataOpRet))
        emit showMsgBoxSig(m_GuiString->uiHints(GUIString::WriteMetaDataErrHint), GUIString::HintDlg);
//    {
//        emit ShowUpFileOpRetSign(QString::fromStdString(m_SysMsgs[Ui::errorMsg]));
//        return updateMetaDataOpRet;
//    }
                                      ...
                                      QString startTime, QString stopTime, qint64 price, QString sharerAgentNodeID)
{
    std::string ret = m_BackRPC->SendOperationCmd(GUIString::GetSharingCodeCmd,
            std::vector<QString>{QString::number(objID), fileName,
                                 ...
                                 receiver, startTime, stopTime, QString::number(price),
