...
func (srv *guiserver) parseAndHandle(jsonBuff []byte) {
	...
	if err := json.Unmarshal(jsonBuff, &opCmdJson); err != nil {
		...
		srv.watingForOperationRet(opCmdJson)
	}
}
...
func (srv *guiserver) watingForOperationRet(req OpRequestMsg) {
	...
	if req.OpCmd != "elephant_getTotalBalance" && req.OpCmd != "elephant_getBalance" && req.OpCmd != "elephant_getSignedCs" {
		logInfo(fmt.Sprintf("New incoming operation: %s", req.OpCmd))
	}
	...
	for {
		...
		case ret := <-retStr:
			if req.OpCmd != "elephant_getTotalBalance" && req.OpCmd != "elephant_getBalance" && req.OpCmd != "elephant_getSignedCs" {
				logInfo(fmt.Sprintf("handleRPCOperation: %s, result: %s", req.OpCmd, ret))
			}
			srv.returnOpRet(req, ret)
			return

		case err := <-errStr:
			logInfo(fmt.Sprintf("handleRPCOperation: %s, result: failed, error: %s", req.OpCmd, err))
...
func (srv *guiserver) returnOpRet(req OpRequestMsg, str string) {
	ret := BackendMsg{SysMessage[opRspMsg], req.OpCmd, str}
	srv.sendMsg2GUI(ret)
}
...
func sendRate(rate int32) {
	ret := BackendMsg{SysMessage[rateMsg], "", strconv.FormatInt(int64(rate), 10)}
	srv.sendMsg2GUI(ret)
}
